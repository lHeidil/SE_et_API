...
{ 
public: 
  CConsole() { ::AllocConsole(); freopen("CONOUT$","wb",stdout); } 
...
private:
  ...
  void SetDefProc(LPCTSTR ClsName) // à utiliser seulement après la création de la fenêtre !
  {
    if(ClsName==DefClsWind) return; // ce n'est pas une classe utilisateur: il n'y a rien à modifier
    defproc=(WNDPROC)(LONG_PTR)::GetWindowLongPtr(m_hWnd,GWLP_WNDPROC);
    if(defproc==WndProc) defproc=::DefWindowProc; // eviter une future boucle infinie
    else ::SetWindowLongPtr(m_hWnd,GWLP_WNDPROC,(LONG)(LONG_PTR)WndProc); // sinon hacker la WndProc
    ::SetWindowLongPtr(m_hWnd,GWLP_USERDATA,(LONG)(LONG_PTR)this); // à utiliser dans WndProc
  }
public:
  // getters/setters
  ...
  // helpers : fabrique un RECT à partir de 4 LONGs
    ...
    UINT WndStyle=DEF_STYLE, UINT WndExStyle=0, LPCTSTR ClsName=0):m_hWnd(0), defproc(::DefWindowProc), is_main(false)
  {
    ...
    if(!ClsName) ClsName=DefClsWind;
    if(!pR) pR=&DefRect;
    m_hWnd=::CreateWindowEx(WndExStyle,ClsName,name,WndStyle,pR->left,pR->top,pR->right,pR->bottom,
    ...
    defproc(::DefWindowProc), is_main(true)
  {
    copy_this=this;
    if(!pR) pR=&DefRect;
    m_hWnd=::CreateWindowEx(WndExStyle,LocCls,name,WndStyle,pR->left,pR->top,pR->right,pR->bottom,
